...
function casValidate($cas) {
	...
	if (isset($_GET['ticket'])) {
		...
		if (isset($cas['validate'])) {
			...
			$result = SimpleSAML_Utilities::fetch($cas['validate'] . $paramPrefix . 'uip_ticket=' . $ticket . '&service=' . urlencode($service));
			$res = preg_split("/\r?\n/", $result);
			...
			$result = SimpleSAML_Utilities::fetch($cas['serviceValidate'] . $paramPrefix . 'uip_ticket=' . $ticket . '&service=' . urlencode($service));
